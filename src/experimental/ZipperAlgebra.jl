...
"""
function _zm_graft_children_masked!(out::WriteZipperCore{V,A},
                                     src::WriteZipperCore{V,A},
                                     range::ByteMask) where {V,A}
...
returns.
"""
function _zm_merge_n!(policy::ZipperMergePolicy,
                      zs::AbstractVector{<:WriteZipperCore{V,A}},
                      active::UInt64,
                      out::WriteZipperCore{V,A}) where {V,A}
    N = length(zs)
    @assert N >= 1 && N <= 64
    @assert active >> N == 0
    ...
    while true  # 'ascend: loop
        ...
        while !break_merge
            ...
            if frontier == active
                ...
                # combine values
                let vals = (wz_is_val(zs[i+1]) ? wz_get_val(zs[i+1]) : nothing
                             for i in active_bits())
                    combined = _zm_combine_n(policy, vals)
                    combined !== nothing && wz_set_val!(out, combined)
                end
            ...
            elseif cnt == 1
                ...
                if next_byte === nothing
                    _zm_on_single!(policy, zs[i+1], frontier,
                                   from_range(Int(a):255), out)
                    break_merge = true
                else
                    _zm_on_single!(policy, zs[i+1], frontier,
                                   from_range(Int(a):Int(next_byte)-1), out)
...
"""
function wz_join_n!(out::WriteZipperCore{V,A},
                    zs::AbstractVector{<:WriteZipperCore{V,A}}) where {V,A}
    isempty(zs) && return
...
"""
function wz_meet_n!(out::WriteZipperCore{V,A},
                    zs::AbstractVector{<:WriteZipperCore{V,A}}) where {V,A}
    isempty(zs) && return
...
"""
function wz_subtract_n!(out::WriteZipperCore{V,A},
                         zs::AbstractVector{<:WriteZipperCore{V,A}}) where {V,A}
    isempty(zs) && return
